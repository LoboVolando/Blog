    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message = 'Read "{}" at {}\n\n{}\'s comments: {}'.format(post.title,
                                                                     ...
                                                                     cd['name'],
                                                                     cd['comments'])
            send_mail(subject, message, 'kudud@yandex.ru.com', [cd['to']])
            sent = True
...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            # results = Post.objects.annotate(search=SearchVector('title', 'body'),).filter(search=query)  # без вектора
            # search_vector = SearchVector('title', 'body')  # вектор без весов

            search_vector = SearchVector('title', weight='A') + SearchVector('body', weight='B')
            # вектор с приоритетом на заголовок

            search_query = SearchQuery(query)

            # results = Post.objects.annotate( # вектор без весов
